...
class CJTExtensions extends CJTHookableClass {
	...
	* 
	*/
	const PREFIXS = 'ecme-,easy-code-manager-';
		...
		foreach ($this->getExtensions() as $class => $extension) {
			...
			if ($extension['definition']['primary']['loadMethod'] == 'auto') {
				...
				if ($frameworkVersion->getMajor() < $extensionVer->getMajor()) {
					...
					$extension['incompatibleMessage']['msg'] = cssJSToolbox::__('Extension is required CJT Framework Version higher than currently installed, CJT need to get updated!!!');
					$extension['incompatibleMessage']['flag'] = cssJSToolbox::__('Aborted');
					...
					if ($frameworkVersion->getMajor() > $extensionVer->getMajor()) {
						$extension['incompatibleMessage']['msg'] = cssJSToolbox::__('Extension is required old CJT Framework Version than the installed. This extension might need to get update. Please check if this extension is currently behaving correctly!!!');
						$extension['incompatibleMessage']['flag'] = cssJSToolbox::__('Ignored');
						// Add to incomaptibility list.
						$this->incompatibilies[$pluginPath] = $extension;
					}
	...
	*/
	public function processIncompatibles() {
		...
		// INitialize.
		$message = cssJSToolbox::__('CJT detects incompatible installed extensions, listed below with status message for every extension:');
		$list = '';
