...
class CJTServicesMVCView {
    ...
    */
    public function __construct(CJTServicesMVCController & $controller) 
    {
        ...
        $this->path =   $config['path'] . DIRECTORY_SEPARATOR . 'Views' . 
                        DIRECTORY_SEPARATOR . 
                        str_replace( '/', DIRECTORY_SEPARATOR, $viewConfig[ 'path' ] );
                        
        $this->url = "{$config[ 'url' ]}/Views/{$viewConfig[ 'path' ]}/";
        
        # Push default resource location
        $this->resourcePoints[''] = $this->url;
        
        # Shared Resources Path
        $sharedResources =& $config['resources']['shared'];
        
        foreach ($sharedResources as $srname => $sr)
        {
            $this->resourcePoints[$srname] = "{$config['url']}/{$sr['path']}";
        }
    ...
    */
    public function dispatch() 
    {
        ...
        $this->output = $this->getTemplate(
            ...
            (isset($route[ 'template' ]) && $route['template']) ? 
             $route[ 'template' ] :
             $route[ 'action' ]
    ...
    * 
    * @param mixed $name
    * @param mixed $resPoint
    */
    public function getResUrl($name, $resPoint = '') 
    {
        return "{$this->resourcePoints[$resPoint]}/{$name}";
    }
    ...
    * @param mixed $name
    */
    public function getScriptUrl($name, $resPoint = '') 
    {
        return $this->getResUrl("{$name}.js", $resPoint);
    }
    ...
    * @param mixed $name
    */
    public function getStyleUrl($name, $resPoint = '') 
    {
        return $this->getResUrl("{$name}.css", $resPoint);
    }
