	...
	*/
	public function get($name) {
		...
		if (!isset($this->params[$key])) {
			echo cssJSToolbox::__("Parameter {$name} is not found!");
		}
		else {
	...
	*/
	public function loadString($strings, $content) {
		...
		foreach ($this->getParams() as $name => $param) {
			...
			// Validate type!                                                                                                                                           
			if (!$param->validate()) {
				echo cssJSToolbox::__("Invalid Shortcode parameter: {$type->getMessage()}");
			}
		...
		if (!empty($strings)) {
			echo cssJSToolbox::__("Invalid Shortcode parameter(s) has been passed! Please check parameters name.");
		}
		return $this;
	}
