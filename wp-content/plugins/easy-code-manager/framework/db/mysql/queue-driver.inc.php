...
class CJTMYSQLQueueDriver extends CJTHookableClass {
    ...
    * put your comment there...
    * 
    * @param mixed $createStmt
    * @param mixed $tableName
    */
    public function createTable($createStmt, & $tableName)
    {
        
        $result = $this->exec($createStmt, $createStmt);
                                               
        // Check table existance
        preg_match('/CREATE\s+TABLE\s+(IF\s+NOT\s+EXISTS\s+)?\`([a-zA-Z0-9_-]+)\`/i', $createStmt, $createTable);
        
        $tableName = $createTable[2];
	...
	* @param mixed $query
	*/
	public function exec($query, & $rQuery = null) 
    {
        
		// Initialize!
		$query = $this->resolveTableName($query);
        
        // Return copy of resolved query
        $rQuery = $query;
    ...
    */
    public function showTables()
    {
        ...
        $result = $this->wpdb->get_results('SHOW TABLES;', ARRAY_N);
        
        if (!$result)
        {
            throw new Exception('Unexpected Error ... Could not read database tables!!!');
        }
