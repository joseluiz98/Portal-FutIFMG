...
define(
    'CJTOOLBOX_ACTIVE_PROFILE',
    getenv('ecm-active-profile') ? 
    getenv('ecm-active-profile') :
    CJTOOLBOX_PROFILE_PRODUCTION
);

define(
    'CJTOOLBOX_ACTIVE_SERVICES_API_PROFILE',
    getenv('ecm-active-services-api-profile') ? 
    getenv('ecm-active-services-api-profile') :
    CJTOOLBOX_PROFILE_PRODUCTION
);
...
class cssJSToolbox extends CJTHookableClass 
{
	...
	* 
	*/
	const CJT_WEB_SITE_DOMAIN = 'easy-code-manager.com';
    ...
    */
    public static function __( $text )
    {
        ...
        if (!empty($args))
        {
            ...
            array_unshift($args, $localTxt);
        
            // Replacements
            $localTxt = call_user_func_array('sprintf', $args);
        }
	...
	*/
	protected function __construct() 
    {
        ...
        // When in development stage display everything
    ...
    * 
    */
    public function getServiesClientAPI()
    {
        // Services API Address would be Local in Development
        // ,and CJT Web Site on Production
        $apiUrl = ( CJTOOLBOX_ACTIVE_SERVICES_API_PROFILE == CJTOOLBOX_PROFILE_PRODUCTION) ?
                    self::getCJTWebSiteURL() :
                    home_url();
        
        return CJTServicesClient::getInstance($apiUrl);
    }
