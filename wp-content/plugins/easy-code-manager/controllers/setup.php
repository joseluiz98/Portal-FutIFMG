...
class CJTsetupController extends CJTAjaxController {
	...
	*/
	protected function licenseAction() 
    {
        ...
        $state['attrs'] = $model->getProductTypeAttrs($state['component']);
        
		// CJT Store Action Request
		$state['response'] = $model->dispatchLicenseAction($action, $state['component'], $state['license'], $state['attrs']);
