        ...
        if ($block->get('id')) {
            if ($block->get('state') == 'active') {
                ...
                if (($this->options['force'] == 'true') || !in_array($block->id, $coupling->getOnActionIds())) {
                    ...
                    $replacement = apply_filters(CJTPluggableHelper::ACTION_SHORTCODE_BLOCK_LINK_TEMPLATES, $replacement, $block);
                }
            }
        }
        else 
        { // Invalid Shortcode block query!
            $replacement = cssJSToolbox::__('Could not find block specified! Please check out the Shortcode parameters.');
        }
