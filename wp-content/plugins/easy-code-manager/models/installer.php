...
class CJTInstallerModel {
	...
	*/
	public function getOperations() 
    {
        ...
        $operations[CJTPlugin::DB_VERSION] = apply_filters(
            CJTPluggableHelper::FILTER_INSTALLER_OPERATIONS,
            $operations[CJTPlugin::DB_VERSION],
            $this->isUpgrade()
        );
            ...
            ($operation['state'] != self::OPERATION_STATE_INSTALLED))
        {
            ...
            switch ($type) 
            {
                ...
                case 'upgrade':
                    ...
                    cssJSToolbox::import($vOperations['upgrader']['file']);
                    
                    $installer = new $vOperations['upgrader']['class']();
            ...
            $installer->{$rOperation['name']}();
            
            $result = array
            (
                'state' => self::OPERATION_STATE_INSTALLED
            );
